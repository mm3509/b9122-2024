...
def is_press_release(url):
    ...
    span_tags = soup.select("span.category_release")

    if 0 == len(span_tags):
        return False

    for tag in span_tags:
        if "press release" == tag.get_text().lower().strip():
            return True
...
def crawl(seed_url, sleep_delay=0):
    ...
    while len(queue) > 0:
        ...
        # this is a Last-In-First-Out queue (LIFO), or "depth-first-search".
        url = queue.pop()
